...
export async function runDemoSeed(opts: SeedOptions = {}): Promise<SeedResult> {
  ...
  if (opts.hardReset) {
    ...
    // we own. Use the broad `seed_` prefix so previous seed runs (which
    // used random suffixes like `seed_<digits>`) also get cleaned up.
    //
    // Schema cascades:
    //   locations → conversations → messages/notes
    //   locations → contacts → conversations → messages/notes
    // …so deleting locations is enough to wipe everything below.
    //
    // assignee_id / author_id columns on conversations/messages/notes
    // reference users.id WITHOUT cascade, so the user delete must come
    // AFTER the locations delete, when nothing references the seed users
    // any more.
